...
export interface LtaTrainServiceAlertsResponse {
  // LTA returns a single status object here, not an array.
...
export interface LtaPlatformCrowdForecastEntry {
  Station: string;
  Interval: Array<{ Start: string; CrowdLevel: CrowdLevel }>;
}

export interface LtaPlatformCrowdForecastResponse {
  // LTA nests forecast entries by date, each holding its own list of stations.
  value: Array<{ Date: string; Stations: LtaPlatformCrowdForecastEntry[] }>;
}
