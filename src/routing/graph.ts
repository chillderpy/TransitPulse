 ...
 * haversine distance anywhere below). Singapore's rail operators don't
 * publish a granular per-segment timetable - MRT runs on headway, not a
 * fixed inter-station schedule - so each line's minutes-per-hop is instead
 * derived from that line's own officially reported end-to-end distance and
 * travel time, rounded to a clean half-minute:
 *   NSL: ~45km  / ~60min / 27 stations (26 hops) => ~2.31 -> 2.5 min/hop
 *   EWL: 57.2km / 77min  / 35 stations (34 hops) => ~2.26 -> 2.5 min/hop
 *   CCL: 39.5km / ~60min / 33 stations (32 hops) => ~1.88 -> 2.0 min/hop
 *   DTL: 42km   / 69min  / 35 stations (34 hops) => ~2.03 -> 2.0 min/hop
 *   TEL: 40.1km / 65min  / 27 stations (26 hops) => 2.5 min/hop (exact)
 *   NEL: 22km   / 35min  / 17 stations (16 hops) => ~2.19 -> 2.0 min/hop
 * The Changi Airport branch (Tanah Merah - Expo - Changi Airport) is
 * modelled at the EWL rate since it's operated as part of that line.
 *
